...
@Injectable()
export class ImprimirContratosService extends FichasDataService implements Resolve<any>
{

	onKardexUrlChanged: BehaviorSubject<any> = new BehaviorSubject(null);
    onComboEntidadChanged: BehaviorSubject <any[]> = new BehaviorSubject(null);
    ...
    getEmpleadosByEntidadId(idEntidad :number): Promise<any> {
        return new Promise((resolve, reject) => {
            this.cargando = true;
            this._httpClient.get(`/api/v1/contratos-empleados/empleados/${idEntidad}`, true)
				.subscribe((response: any) => {
					this.cargando = false;
					if (response.status == 200) {
                        this.onComboEntidadChanged.next(response.data);
                    } else {
                        this.onComboEntidadChanged.next(null);
