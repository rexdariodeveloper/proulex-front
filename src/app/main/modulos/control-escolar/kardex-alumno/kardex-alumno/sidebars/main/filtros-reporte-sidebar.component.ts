...
export class FiltrosKardexAlumnoSidebarComponent implements OnInit, OnDestroy {
    ...
    constructor(
        private _FichasDataService: KardexAlumnoService,
    ...
    filtar(form: FormGroup) {
        if (form.valid) {
            ...
            if(!!form['controls']?.tipo?.value)
                ...
                this._FichasDataService.existeAlumno(form.value.codigoAlumno,false);
        } else {
    ...
    imprimir() {
        ...
        if (form.valid) {
            this._FichasDataService.existeAlumno(form.value.codigoAlumno,true);
