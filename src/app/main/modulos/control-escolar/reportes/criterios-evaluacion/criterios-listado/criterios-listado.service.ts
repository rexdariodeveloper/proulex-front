...
@Injectable()
export class CriteriosGrupoService extends FichasDataService implements Resolve<any>
{
	...
	getZip(json: any): Promise<any> {
        this.cargando = true;
        return new Promise((resolve, reject) => {
            this._httpClient.post(json, '/api/v1/criterios-evaluacion/template/generarReporte', true)
                .subscribe((response: any) => {
                	let blob = this.b64toBlob(response.data.archivo, this.obtenerContentType(response.data.extension));
                	this._archivoService.descargarArchivoResponse(blob,response.data.extension,'Asignacion_Clases.zip');
                    this.cargando = false;
                }, err => {
