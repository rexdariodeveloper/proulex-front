...
@Injectable()
export class InasistenciasService extends FichasDataService implements Resolve<any>
{
	...
	getCiclos(sedes: any): Promise<any> {
		return new Promise((resolve, reject) => {
			this.cargando = true;
			this._httpClient.post({sedes: sedes}, '/api/v1/inasistencias/getCiclos', true).subscribe((response: any) => {
				this.cargando = false;
	...
	getFechas(filtros: any): Promise<any> {
		return new Promise((resolve, reject) => {
			this.cargando = true;
			this._httpClient.post(filtros, '/api/v1/inasistencias/getFechas', true).subscribe((response: any) => {
				this.cargando = false;
				if (response.status == 200)
