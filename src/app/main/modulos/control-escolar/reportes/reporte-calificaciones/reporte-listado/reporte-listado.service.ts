...
@Injectable()
export class ReporteCalificacionesService extends FichasDataService implements Resolve<any> {
  private rutaBuscadorLocalStorage: string = 'ficha-listado-buscador';
  private buscadorMap: {[ruta:string]: string} = {};
  url: string = '/api/v1/captura_calificaciones/';
  ...
  getPlantelesByFiltros(data: any): Promise <any> {
    return new Promise((resolve, reject) => {
      this.cargando = true;
      this._httpClient.post(data, this.url + 'listados/planteles', true).subscribe((response: any) => {
        this.cargando = false;
  ...
  getPlantelesAlumnosByFiltros(data: any): Promise <any> {
    return new Promise((resolve, reject) => {
      this.cargando = true;
      this._httpClient.post(data, this.url + 'listados/planteles_alumno', true).subscribe((response: any) => {
        this.cargando = false;
        if(response.status == 200)
  ...
  getProgramacionesByFiltros(data: any): Promise <any> {
    return new Promise((resolve, reject) => {
      this.cargando = true;
      this._httpClient.post(data, this.url+ 'listados/programaciones', true).subscribe((response: any) => {
        this.cargando = false;
        if (response.status == 200)
  ...
  getModalidadesByFiltros(data: any): Promise <any> {
    return new Promise((resolve, reject) => {
      this.cargando = true;
      this._httpClient.post(data,this.url + 'listados/modalidades', true).subscribe((response: any) => {
        this.cargando = false;
        if (response.status == 200)
  ...
  getFechasByFiltros(data: any): Promise <any> {
    return new Promise((resolve, reject) => {
      this.cargando = true;
      this._httpClient.post(data, this.url + 'listados/fechas', true).subscribe((response: any) => {
        this.cargando = false;
        if (response.status == 200) {
