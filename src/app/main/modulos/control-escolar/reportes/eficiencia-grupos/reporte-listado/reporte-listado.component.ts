...
export class ReporteListadoComponent {
    ...
    config: FichaListadoConfig = {
        ...
        columns: [
            { name: "Sede", title: "Sede", class: "mat-column-160", centrado: false, type: null, tooltip: true },
            { name: "Grupo", title: "Grupo", class: "mat-column-160", centrado: false, type: null, tooltip: true }
        ],
        displayedColumns: ['Sede', 'Grupo'],
        columasFechas: [],
        listadoMenuOpciones: [],
        listadoAcciones: []
    ...
    opciones: ListadoMenuOpciones[] = [
        { title: 'Descargar Reporte', icon: 'play_for_work', tipo: FichaListadoConfig.EXCEL, url: "/api/v1/eficiencia-grupos/exportar/xlsx" }
    ];
                ...
                if (datos?.sedes) {
                    ...
                    if (datos?.horarios) {
                        datos.horarios.forEach(horario => {
                            this.config.columns.push({ name: horario, title: horario, class: "mat-column-flex", centrado: true, type: null, tooltip: true });
                            this.config.displayedColumns.push(horario);
                        });
                    }
                }
            });
