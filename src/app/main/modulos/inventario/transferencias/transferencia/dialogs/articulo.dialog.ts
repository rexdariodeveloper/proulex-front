...
export class ArticuloDialogComponent {
	...
	setData(data: ArticuloDialogData) {
		...
		setTimeout(() => {
			...
			this.form.controls.articulo.valueChanges.pipe(takeUntil(this._unsubscribeAll)).subscribe(() => {
				let articuloId: number = this.form.controls.articulo.value ? this.form.controls.articulo.value.id : null;

				this.form.controls.existencia.setValue(this.getExistencia(this.localidadId, articuloId));
			});
		});
	}
	...
	aceptar(): void {
		if (this.form.valid) {
			...
			} else if (registroActualizar.existencia < registroActualizar.cantidad) {
				this._matSnackBar.open(this.translate.instant('CANTIDAD_EXISTENCIA'), 'OK', { duration: 5000 });
			} else {
