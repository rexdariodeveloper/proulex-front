...
export class FiltrosReporteSidebarComponent implements OnInit, OnDestroy {
    ...
    ngAfterViewInit(): void {
        ...
        form.controls['fechaInicio'].setValue(fechaActual);
        form.controls['fechaFin'].setValue(fechaActual);

        form.controls['tipo'].valueChanges.pipe(takeUntil(this._unsubscribeAll)).subscribe( (data) => {
            if(data.id){
                form.controls['articulos'].enable();
                form.controls['series'].disable();
                form.controls['series'].setValue(null);
            }
            else {
                form.controls['articulos'].disable();
                form.controls['series'].enable();
                form.controls['articulos'].setValue(null);
            }
        });
    }
    ...
    filtar(form: FormGroup) {
        if (form.valid) {
            ...
            this._FichasDataService.getArchivoURL('/api/v1/valuacion/pdf/',form.value);
        } else {
