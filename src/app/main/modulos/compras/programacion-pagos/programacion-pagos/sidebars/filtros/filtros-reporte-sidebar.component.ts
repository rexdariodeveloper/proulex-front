...
export class FiltrosReporteSidebarComponent implements OnInit, OnDestroy {
    ...
    @Input() service: FichasDataService = null;
    ...
    ngAfterViewInit(): void {
        ...
        this.form.form.form.controls['fechaFactura'].setValue(fechaActual);
    }
    ...
    filtar(form: FormGroup) {
        if (form.valid) {
            ...
            this.service.getDatosFiltros(form.value);
        } else {
    ...
    selectedOpcion(opc: any) {
        ...
        if (form.valid) {
            if (FichaListadoConfig.EXCEL == opc.tipo) {
                ...
                this.service.getExcelConFiltros(opc.url, form.value);
